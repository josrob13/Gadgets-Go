...
Además de contar con esos escenarios que serán los principales, y donde ocurra la trama principal del videojuego, se ha adaptado el \textit{asset} que servía de menú principal, y se ha creado una escena extra
al comenzar el juego, donde el usuario entablará conversación con el que será, según la narrativa, su ``jefe de la agencia detectivesca''. Este le preguntará su nombre, y le dará una breve introducción sobre por qué
está ahí y qué debe lograr. La escena se ha creado exclusivamente para esta introducción y cuenta con pocos elementos 3D, pero todos ellos han sido recogidos de fuentes públicas. Como se observa, el propio diálogo de la introducción
se basa en el sistema de diálogos actual, que no necesita de una misión para su correcto funcionamiento.

\huecofigura[7cm]{Escena introductoria con el jefe de A.A.D.S}
...
Estos paneles se orientan automáticamente hacia el usuario para que siempre queden de frente.
  ...
  \item \textbf{Interacción mediante puntero láser:} la selección de respuestas y opciones se realiza con un puntero láser que se emite desde el mando, una metáfora de interacción ya consolidada y muy intuitiva en RV, que se detallará en el apartado de arquitectura (\ref{sec:arquitectura}).
\end{enumerate}
...
\huecofigura[7cm]{Panel de diálogo y cuestionario de evaluación renderizados como interfaz espacial (\textit{World Space}) dentro de la escena.}
...
\huecofigura[6cm]{Indicador de monedas (\textit{SpyCoins}) acompañando al usuario en la zona inferior del campo de visión.}
...
La filosofía de diseño hereda la del primer trabajo (modularidad, desacoplamiento y patrones sólidos), con un principio añadido que se impuso desde el inicio: \textbf{la lógica de las analíticas de tracking debe permanecer completamente separada del sistema de diálogos}, comunicándose con él únicamente a través de eventos.

\subsection{Tecnologías Unity, Meta XR SDK y OpenXR}

\subsubsection{Investigación sobre la base tecnológica}
...
Su principal aportación a la accesibilidad del desarrollo son los \textbf{Building Blocks}: un sistema de bloques prefabricados que se arrastran a la escena y resuelven de un plumazo tareas de fontanería que de otro modo llevarían días, como el \textit{rig} de cámara del jugador (\texttt{OVRCameraRig}, con sus anclajes de cabeza y manos), la configuración de los mandos, el \textit{passthrough} o el conjunto de la locomoción.
...
Sobre esa base, la integración con la Realidad Virtual se apoya en las siguientes tecnologías utilizadas:
...
Actuar sobre él, en lugar de sobre una API propietaria cerrada, garantiza que el proyecto no quede atado a un único fabricante y facilita su portabilidad futura a otros dispositivos.
...
\\
Se ha mantenido, además, el \textit{Universal Render Pipeline} (URP) del primer trabajo, ahora configurado para un renderizado eficiente, una decisión clave para el presupuesto de rendimiento descrito en el capítulo anterior.
